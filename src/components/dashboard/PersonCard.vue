...
// const maskedName = computed(() => maskNameParts(props.person.Name));
...
const formattedDate = computed(() => {
  if (!props.person.Date) return "";
  const date = new Date(props.person.Date);
...
function onCardClick() {
  ...
  if (!props.person?.Number) return;
  router.push({
    name: "person-detail",
    params: { id: String(props.person.Number) },
    query: {
  ...
  <div class="col-6 col-sm-4 col-md-3 col-lg-2 mb-3">
    ...
    >
      ...
      <div class="p-2 text-center">
        ...
        <p v-if="formattedDate" class="small text-muted mb-0">
          {{ t("dashboard.testDateLine", { date: formattedDate }) }}
        </p>
