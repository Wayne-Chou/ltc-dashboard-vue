...
const overviewText = computed(() => {
  if (!sortedPersons.value.length && !hasNoData.value) return ''
...
const modalRootId = computed(() =>
  props.mode === 'risk' ? 'participantsModal' : 'participantsLevelModal',
)

const desktopFilterId = computed(() =>
  props.mode === 'risk' ? 'modalFilterBtnsDesktop' : 'modalLevelFilterBtnsDesktop',
)

const mobileDropdownId = computed(() =>
  props.mode === 'risk' ? 'modalFilterDropdownMobile' : 'modalLevelFilterDropdownMobile',
)

const personContainerId = computed(() =>
  props.mode === 'risk' ? 'modalPersonContainer' : 'modalLevelPersonContainer',
)

const mobileDropdownButtonId = computed(() =>
  props.mode === 'risk' ? 'modalFilterDropdownMobileBtn' : undefined,
)
